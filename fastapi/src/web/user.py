...
ASCCESS_TOKEN_EXPIRE_MINUTES = 30
...
async def create_access_token(
    ...
    """username과 password를 OAth 양식에서 꺼내고
        JWT 접근 토큰을 반환한다"""
    user = service.auth_user(form_data.username, form_data.password)
    if not user:
        unauthed()
    expire = timedelta(minutes=ASCCESS_TOKEN_EXPIRE_MINUTES)
    access_token = service.create_access_token(
        data={"sub": user.name}, expires=expire
    )
